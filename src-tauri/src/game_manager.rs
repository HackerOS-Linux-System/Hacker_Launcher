...
pub struct Game {
    ...
    /// Android package ID (e.g. `com.mojang.minecraftpe`), used only when
    /// `runner == "Android"`. Waydroid launches/stops apps by package ID,
    /// not by file path — `exe` still holds the `.apk` path (for
    /// (re)installing), but this is what's actually needed to launch.
    #[serde(default)]
...
impl GameManager {
    ...
    /// Forcefully stops a running game. For every runner except Android,
    /// this sends SIGKILL to the tracked pid — the background watcher
    /// thread spawned in `launch_game` then notices the exit, updates
    /// playtime, and removes it from `running`. Android games run inside
    /// the Waydroid container rather than as a direct child process (see
    /// `launch_android_game`), so there's no local pid to kill; instead
    /// this calls `waydroid app stop` and finalizes playtime here.
    ///
    /// Android games also have a background poll thread (started in
    /// `launch_android_game`) that can independently notice the app
    /// exited on its own (closed from inside Android, crashed, etc.) and
    /// race to finalize the same entry — so removal from `running` here
    /// is done atomically via `HashMap::remove`, and whichever of the two
    /// (this explicit Stop, or that poll thread) actually gets `Some(_)`
    /// back is the one that updates playtime; the loser sees `None` and
    /// does nothing further, rather than double-counting playtime or
    /// writing stale data.
    ///
    ...
    pub fn stop_game(&self, name: &str) -> Result<()> {
        // Up-front presence check purely for a clear error message if
        // there's nothing tracked at all; the actual removal happens
        // atomically further down for the reason described above.
        {
            let running = self.running.lock().unwrap();
            if !running.contains_key(name) {
                bail!("Game is not currently running: {}", name);
            }
        }
        ...
        let is_android = self
            ...
            .unwrap_or_default()
            .iter()
            .any(|g| g.name == name && g.runner == "Android");
        ...
        if is_android {
            let android_package = self
                ...
                .into_iter()
                .find(|g| g.name == name)
                .map(|g| g.android_package)
                .unwrap_or_default();
            // Best-effort regardless of the race outcome below — if the
            // poll thread already noticed the app was gone, this will
            // likely just no-op or fail harmlessly against an
            // already-stopped app, which isn't worth surfacing as an error.
            let stop_result = crate::android_manager::stop_apk(&android_package);

            let Some(entry) = self.running.lock().unwrap().remove(name) else {
                // The poll thread in `launch_android_game` already
                // finalized this game between our presence check above
                // and here — nothing left for this call to do.
                return Ok(());
            };
            let elapsed = (Local::now() - entry.started_at).num_seconds().max(0) as u64;
            if let Ok(content) = fs::read_to_string(&self.games_file) {
                if let Ok(mut games) = serde_json::from_str::<Vec<Game>>(&content) {
                    ...
                    if let Ok(new_content) = serde_json::to_string_pretty(&games) {
                        let _ = fs::write(&self.games_file, new_content);
                    }
                }
            }
            return stop_result;
        ...
        let pid = self.running.lock().unwrap().get(name).map(|e| e.pid).unwrap_or(0);
    ...
    pub fn launch_game(
        ...
        // Validation
        if game.runner != "Steam" && game.runner != "Android" && !std::path::Path::new(&game.exe).exists() {
            bail!("Executable does not exist: {}", game.exe);
        }
        if game.runner == "Steam" && game.app_id.is_empty() {
            bail!("Steam App ID not set");
        }
        if game.runner == "Android" && game.android_package.trim().is_empty() {
            bail!("Android package name not set — install the APK (or enter the package ID) first");
        }

        // Android apps run inside the Waydroid container rather than as a
        // process this launcher spawns and owns directly, so none of the
        // Wine/Proton prefix setup, env building, or gamescope/command
        // assembly below applies to them — handle them separately and
        // return early.
        if game.runner == "Android" {
            return self.launch_android_game(game, app_handle);
        }
    ...
    /// Launches an Android game via Waydroid. Kept separate from the main
    /// `launch_game` body above because Android apps don't fit that
    /// function's model at all: no exe path to spawn directly, no Wine
    /// prefix, no gamescope/env-var pipeline, and — crucially — the
    /// process this function spawns (`waydroid app launch`) exits almost
    /// immediately once it hands the request off to the container, rather
    /// than being the game itself. That means the other runners' "watcher
    /// thread blocks on the child's real exit" pattern doesn't apply
    /// directly here.
    ///
    /// Instead, this spawns its own poll loop that periodically asks
    /// Waydroid whether the app's process is still alive inside the
    /// container (`android_manager::is_app_running`, essentially a
    /// `pidof` check) and finalizes playtime + clears `running` as soon as
    /// it isn't — so a game closed from *inside* Android itself (not via
    /// this launcher's Stop button) still gets tracked as stopped and its
    /// playtime recorded, instead of only reacting to an explicit Stop
    /// click. This is still an approximation, not a real exit event:
    /// detection lags by roughly the poll interval, and it inherits
    /// whatever blind spots `pidof`-style matching has (see that
    /// function's doc comment). `stop_game`'s Android branch and this
    /// poll loop race safely against each other — see `stop_game`'s doc
    /// comment for how.
    fn launch_android_game(&self, mut game: Game, app_handle: tauri::AppHandle) -> Result<()> {
        use tauri::Emitter;
        ...
        let mut child = crate::android_manager::launch_apk(&game.android_package)?;
        // Reap the short-lived launch helper so it doesn't linger as a
        // zombie; we deliberately don't treat its exit as "the game
        // stopped" (see the doc comment above).
        std::thread::spawn(move || {
            let _ = child.wait();
        });
        ...
        {
            let mut running = self.running.lock().unwrap();
            running.insert(game.name.clone(), RunningEntry { pid: 0, started_at });
        }
        let _ = app_handle.emit(
            "game_started",
            serde_json::json!({ "name": game.name, "pid": 0, "started_at": started_at.to_rfc3339() }),
        );

        // Background poll loop: detects the app's *actual* exit inside the
        // Waydroid container instead of only relying on the user clicking
        // Stop. Debounced (two consecutive "not running" reads, ~8s apart)
        // so a single transient `waydroid shell` hiccup right after launch
        // doesn't get misread as the app already having exited.
        let running_map = self.running.clone();
        let games_file = self.games_file.clone();
        let game_name = game.name.clone();
        let android_package = game.android_package.clone();
        let app_handle_watch = app_handle.clone();
        std::thread::spawn(move || {
            // Give the app a moment to actually start before the first
            // check, so a slow cold start inside the container isn't
            // mistaken for "never started".
            std::thread::sleep(std::time::Duration::from_secs(5));

            let mut consecutive_not_running = 0u8;
            loop {
                // If `stop_game` already removed this entry (explicit
                // Stop), our job is done — don't finalize a second time.
                if !running_map.lock().unwrap().contains_key(&game_name) {
                    return;
                }
                if crate::android_manager::is_app_running(&android_package) {
            ...
            // The app looks gone from the container. Atomically claim the
            // running entry — if `stop_game` won the race and already
            // removed it, `remove` returns `None` here and there's nothing
            // left to finalize (avoids double-counting playtime).
            let Some(entry) = running_map.lock().unwrap().remove(&game_name) else {
                return;
            };
            let elapsed = (Local::now() - entry.started_at).num_seconds().max(0) as u64;
            if let Ok(content) = fs::read_to_string(&games_file) {
            ...
            let _ = app_handle_watch.emit("game_stopped", serde_json::json!({ "name": game_name }));
        });
