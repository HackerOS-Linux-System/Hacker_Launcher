...
#[tauri::command]
fn scan_game_dependencies(exe_path: String, prefix: String) -> Result<Vec<tools::DependencyHint>, String> {
    tools::scan_game_dependencies(&exe_path, &prefix).map_err(|e| e.to_string())
}
...
#[tauri::command]
async fn check_protondb(
    state: State<'_, AppState>,
    app_id: String,
) -> Result<Option<tools::ProtonDbInfo>, String> {
    let cache_file = {
        let cfg = state.config.lock().map_err(|e| e.to_string())?;
        cfg.config_dir.join("protondb_cache.json")
    };
    tools::check_protondb(&app_id, &cache_file)
        .await
        .map_err(|e| e.to_string())
}

/// Looks up possible Steam App IDs by name, for games with no App ID on
/// record (e.g. added manually rather than via the Steam library
/// importer) — mainly so ProtonDB lookup has something to key on.
#[tauri::command]
async fn search_steam_appid(query: String) -> Result<Vec<tools::SteamSearchResult>, String> {
    tools::search_steam_appid(&query).await.map_err(|e| e.to_string())
}
...
// ─────────────────────────────────────────────
//  Android (.apk via Waydroid)
// ─────────────────────────────────────────────

#[tauri::command]
fn is_waydroid_available() -> bool {
    android_manager::waydroid_available()
}

/// Reads the package ID out of an `.apk` (via `aapt`/`aapt2`) without
/// installing it, so the Add/Configure dialogs can show and let the user
/// confirm/correct it up front — mirroring how the Steam importer lets you
/// fix a guessed exe path before committing.
#[tauri::command]
fn get_apk_package_name(apk_path: String) -> Result<String, String> {
    android_manager::extract_apk_package_name(&apk_path).map_err(|e| e.to_string())
}

#[tauri::command]
async fn install_apk(apk_path: String, app_handle: tauri::AppHandle) -> Result<String, String> {
    let app_for_run = app_handle.clone();
    let result = tokio::task::spawn_blocking(move || android_manager::install_apk(&apk_path, &app_for_run))
        .await
        .map_err(|e| e.to_string())?;
    match &result {
        Ok(pkg) => notify(&app_handle, "APK installed", &format!("Package: {}", pkg)),
        Err(e) => notify(&app_handle, "APK install failed", &e.to_string()),
    }
    result.map_err(|e| e.to_string())
}
