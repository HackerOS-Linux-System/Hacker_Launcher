...
export default function GamesTab(props: Props) {
  ...
  const [addPrefill, setAddPrefill] = createSignal<{ exe?: string; name?: string; runner?: string } | null>(null);
  ...
  onMount(() => {
    ...
    // Drag & drop: dropping a .exe or .apk anywhere on the window opens
    // Add Game prefilled with that path, instead of requiring Browse.
    const unlistenDrop = getCurrentWebview().onDragDropEvent((event) => {
      ...
      if (payload.type === "enter" || payload.type === "over") {
        ...
        const apkPath = payload.paths.find((p) => p.toLowerCase().endsWith(".apk"));
        if (exePath) {
          const fileName = exePath.split(/[\\/]/).pop() ?? "";
          const guessedName = fileName.replace(/\.exe$/i, "").replace(/[_-]+/g, " ").trim();
          setAddPrefill({ exe: exePath, name: guessedName, runner: "Proton" });
          setShowAdd(true);
        } else if (apkPath) {
          const fileName = apkPath.split(/[\\/]/).pop() ?? "";
          const guessedName = fileName.replace(/\.apk$/i, "").replace(/[_-]+/g, " ").trim();
          setAddPrefill({ exe: apkPath, name: guessedName, runner: "Android" });
          setShowAdd(true);
        } else {
          props.addToast("Drop a Windows .exe or an Android .apk file to add it as a game", "info");
        }
      }
    });
  ...
  return (
    <>
      <Show when={dragActive()}>
        <div class="dropzone-overlay"><span>📥 Drop the .exe or .apk to add it as a game</span></div>
      </Show>
        ...
        <Show
          ...
          fallback={
            <div class="empty-state">
              ...
              <span style={{ "font-size": "11px" }}>Tip: drag & drop a .exe or .apk file anywhere onto the window</span>
            </div>
          }
        >
          <Show
      ...
      <Show when={showAdd()}>
        <AddGameModal
          ...
          initialRunner={addPrefill()?.runner}
        ...
        <div class="modal-overlay" onClick={() => !importingSteam() && setShowSteamImport(false)}>
          <div class="modal" style={{ "min-width": "560px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ color: "var(--text-muted)", "font-size": "12px" }}>
              ...
              When available, the main executable is read straight out of Steam's own local launch-config
              cache (tagged "steam metadata" below); otherwise it falls back to a heuristic guess (tagged
              "heuristic scan") — check and fix the path either way before importing if it looks wrong.
              Imported games use the Steam runner, so Steam itself manages Proton/prefix for them.
            </p>
                    ...
                    {(c) => (
                      <div style={{ padding: "8px", "border-radius": "6px", background: "rgba(255,255,255,0.02)", display: "flex", "flex-direction": "column", gap: "6px" }}>
                        ...
                        <Show when={c.source}>
                          <span
                            class="badge"
                            classList={{ "badge-green": c.source === "steam metadata", "badge-gray": c.source === "heuristic scan" }}
                            style={{ "font-size": "9px", "align-self": "flex-start", "margin-left": "24px" }}
                          >
                            {c.source === "steam metadata" ? "✓ from Steam metadata" : "heuristic guess"}
                          </span>
                        </Show>
                      </div>
                    )}
