...
/** SDL hat direction bitmask (`SDL_HAT_UP` etc.) used to build the `hH.MASK`
 * mapping syntax — a hat's *direction*, not its raw axis number, is what
 * SDL actually keys the mapping on. */
const HAT_MASK: Record<string, number> = { up: 1, right: 2, down: 4, left: 8 };

function eventToCode(e: ControllerInputEvent): string {
  if (e.kind === "button") return `b${e.number}`;
  if (e.kind === "hat" && e.hat_direction) {
    // A D-Pad reported as a hat rather than four separate buttons — SDL
    // encodes this as "h<hat index>.<direction bitmask>" rather than "aN",
    // which is why hats need this separate branch instead of falling
    // through to the plain axis case below.
    return `h${e.hat_index ?? 0}.${HAT_MASK[e.hat_direction]}`;
  }
  return `a${e.number}`;
}
    ...
    <div class="modal-overlay" onClick={props.onClose}>
      <div class="modal" style={{ "min-width": "620px" }} onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ color: "var(--text-muted)", "font-size": "12px" }}>
          ...
          on your pad within 6 seconds. D-Pads reported by the kernel as a "hat" rather than four
          separate buttons are detected automatically and mapped correctly; this is still a
          best-effort helper overall, so double-check the generated string (and the
          auto-detected GUID) before relying on it.
        </p>
