...
export default function ConfigureGameModal(props: Props) {
  ...
  // Android (Waydroid)
  const [apkPath, setApkPath] = createSignal(props.game.exe ?? "");
  const [androidPackage, setAndroidPackage] = createSignal(props.game.android_package ?? "");
  const [detectingPackage, setDetectingPackage] = createSignal(false);
  const [installingApk, setInstallingApk] = createSignal(false);
  const [apkInstallLog, setApkInstallLog] = createSignal("");
  const [waydroidAvailable, setWaydroidAvailable] = createSignal<boolean | null>(null);

  // Steam App ID search (for games with no App ID on record)
  const [showAppIdSearch, setShowAppIdSearch] = createSignal(false);
  const [appIdSearchQuery, setAppIdSearchQuery] = createSignal(props.game.name ?? "");
  const [appIdSearchResults, setAppIdSearchResults] = createSignal<Array<{ app_id: string; name: string }>>([]);
  const [searchingAppId, setSearchingAppId] = createSignal(false);
  ...
  onMount(() => {
    ...
    if (runner() === "Android") {
      invoke<boolean>("is_waydroid_available").then(setWaydroidAvailable).catch(() => setWaydroidAvailable(false));
    }

    // Live progress: winetricks / installer / APK install all stream
    // stdout+stderr line by line via this event instead of only showing a
    // log once finished, so long operations don't look frozen.
    const unlistenOutput = listen<{ source: string; line: string }>("process_output", (event) => {
      ...
      } else if (source === "apk_install") {
        setApkInstallLog((l) => l + line + "\n");
      }
    });
  ...
  // ── Steam App ID search (for games added manually, with no App ID) ──
  const searchAppId = async () => {
    if (!appIdSearchQuery().trim()) return;
    setSearchingAppId(true);
    setAppIdSearchResults([]);
    try {
      const results = await invoke<Array<{ app_id: string; name: string }>>("search_steam_appid", {
        query: appIdSearchQuery().trim(),
      });
      setAppIdSearchResults(results);
      if (results.length === 0) props.addToast("No matching Steam store entries found", "info");
    } catch (e) {
      props.addToast(`Steam search failed: ${e}`, "error");
    } finally {
      setSearchingAppId(false);
    }
  };

  const pickAppId = (id: string) => {
    setAppId(id);
    setShowAppIdSearch(false);
    checkProtonDb();
  };
  ...
  const browseApk = async () => {
    ...
    if (typeof result === "string") {
      setApkPath(result);
      detectPackage(result);
    }
  };

  const detectPackage = async (path: string) => {
    setDetectingPackage(true);
    try {
      const pkg = await invoke<string>("get_apk_package_name", { apkPath: path });
      setAndroidPackage(pkg);
    } catch (e) {
      props.addToast(`Couldn't auto-detect the package name: ${e}. Enter it manually below.`, "info");
    } finally {
      setDetectingPackage(false);
    }
  };

  const installApk = async () => {
    if (!apkPath().trim()) return props.addToast("Select an APK file first", "error");
    setInstallingApk(true);
    setApkInstallLog("");
    try {
      const pkg = await invoke<string>("install_apk", { apkPath: apkPath().trim() });
      setAndroidPackage(pkg);
      props.addToast(`Installed — package: ${pkg}`, "success");
    } catch (e) {
      props.addToast(`APK install failed: ${e}`, "error");
    } finally {
  ...
  const handleSave = async () => {
    ...
    const updated: Game = {
      ...
      exe: runner() === "Android" ? apkPath().trim() : props.game.exe,
      android_package: runner() === "Android" ? androidPackage().trim() : props.game.android_package,
    };
    setSaving(true);
    try {
  ...
  const openDeps = async () => {
    ...
    setWinetricksLog("");
    try {
      const hints = await invoke<DependencyHint[]>("scan_game_dependencies", {
        exePath: props.game.exe,
        prefix: prefix().trim() || props.game.prefix || "",
      });
      setDepHints(hints);
    } catch (e) {
      ...
      <div class="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div class="form-grid">
          ...
          <select value={runner()} onChange={(e) => setRunner(e.currentTarget.value)}>
            <For each={["Native", "Wine", "Proton", "Flatpak", "Steam", "Android"]}>{(r) => <option>{r}</option>}</For>
          </select>
          ...
          <Show when={runner() === "Android"}>
            <label class="form-label">APK File</label>
            <div class="input-group">
              <input type="text" value={apkPath()} onInput={(e) => setApkPath(e.currentTarget.value)} placeholder="Path to the .apk" />
              <button onClick={browseApk}>📁 Browse</button>
            </div>
            ...
            <div class="input-group">
              <input
                ...
                onInput={(e) => setAndroidPackage(e.currentTarget.value)}
                placeholder="e.g. com.example.app"
              />
              <button onClick={() => apkPath() && detectPackage(apkPath())} disabled={!apkPath() || detectingPackage()}>
                {detectingPackage() ? "…" : "🔍 Detect"}
              </button>
            </div>

            <span></span>
            <div style={{ display: "flex", "flex-direction": "column", gap: "6px" }}>
              <Show when={waydroidAvailable() === false}>
                <div style={{ color: "var(--text-muted)", "font-size": "12px" }}>
                  Waydroid wasn't found — install it (see docs.waydro.id) before this can launch.
                </div>
              </Show>
              <div>
                <button onClick={installApk} disabled={installingApk() || !apkPath().trim()}>
                  {installingApk() ? (<><span class="spinner" /> Installing…</>) : "📲 Install APK to Waydroid"}
                </button>
              </div>
              <Show when={apkInstallLog()}>
                <pre style={{
                  background: "rgba(12,12,24,0.8)", border: "1px solid var(--border-subtle)",
                  "border-radius": "8px", padding: "10px", "max-height": "140px", overflow: "auto",
          ...
          <div class="input-group">
            ...
            <button onClick={() => setShowAppIdSearch((s) => !s)} title="Find the App ID by game name">
              🔎 Find by name
            </button>
          </div>
          <Show when={showAppIdSearch()}>
            <span></span>
            <div style={{ display: "flex", "flex-direction": "column", gap: "6px" }}>
              <div class="input-group">
                <input
                  type="text"
                  value={appIdSearchQuery()}
                  onInput={(e) => setAppIdSearchQuery(e.currentTarget.value)}
                  placeholder="Game title to search on Steam"
                  onKeyDown={(e) => { if (e.key === "Enter") searchAppId(); }}
                />
                <button onClick={searchAppId} disabled={searchingAppId() || !appIdSearchQuery().trim()}>
                  {searchingAppId() ? <span class="spinner" /> : "Search"}
                </button>
              </div>
              <Show when={appIdSearchResults().length > 0}>
                <div style={{ display: "flex", "flex-direction": "column", gap: "4px", "max-height": "160px", overflow: "auto" }}>
                  <For each={appIdSearchResults()}>
                    {(r) => (
                      <button
                        onClick={() => pickAppId(r.app_id)}
                        style={{
                          "text-align": "left", background: "rgba(255,255,255,0.03)", padding: "6px 8px",
                          "font-size": "12px", "border-radius": "6px",
                        }}
                      >
                        {r.name} <span style={{ color: "var(--text-muted)" }}>({r.app_id})</span>
                      </button>
                    )}
                  </For>
                </div>
                      ...
                      <div style={{ display: "flex", "align-items": "center", "justify-content": "space-between", gap: "8px", padding: "8px", background: "rgba(255,255,255,0.03)", "border-radius": "8px" }}>
                        <div>
                          <div style={{ "font-weight": 600 }}>
                            {hint.label}
                            <Show when={hint.already_installed}>
                              {" "}
                              <span class="badge badge-green" style={{ "font-size": "10px" }}>already installed</span>
                            </Show>
                          </div>
