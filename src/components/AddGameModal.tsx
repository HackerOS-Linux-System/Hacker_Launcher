...
const RUNNERS = ["Native", "Wine", "Proton", "Flatpak", "Steam", "Android"];
...
export default function AddGameModal(props: Props) {
  ...
  onMount(() => {
    invoke<Array<{ version: string }>>("get_installed_protons")
      ...
      .catch(() => {});
    if (isAndroid() && exe()) detectPackage(exe());
  });

  const isWineOrProton = () => runner() === "Wine" || runner() === "Proton";
  const isAndroid = () => runner() === "Android";

  const detectPackage = async (apkPath: string) => {
    setDetectingPackage(true);
    try {
      const pkg = await invoke<string>("get_apk_package_name", { apkPath });
  ...
  const browseExe = async () => {
    ...
    if (typeof result === "string") {
      setExe(result);
      if (isAndroid()) detectPackage(result);
    }
  };
      ...
      <div class="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div class="form-grid">
          ...
          <Show when={runner() !== "Steam"}>
            <label class="form-label">{isAndroid() ? "APK File" : "Executable"}</label>
            <div class="input-group">
              <input
                type="text"
                value={exe()}
                onInput={(e) => setExe(e.currentTarget.value)}
                placeholder={isAndroid() ? "Select an .apk to install" : "Select or enter path"}
              />
          ...
          <Show when={isAndroid()}>
            ...
            <div class="input-group">
              <input
                ...
                placeholder="e.g. com.example.app (auto-filled from the APK when possible)"
              />
              <button onClick={() => exe() && detectPackage(exe())} disabled={!exe() || detectingPackage()}>
                {detectingPackage() ? "…" : "🔍 Detect"}
              </button>
            </div>
            <span></span>
            <p style={{ color: "var(--text-muted)", "font-size": "12px", margin: "0 0 4px" }}>
              Runs via Waydroid. The APK still needs to be installed into Waydroid before it can be
              launched — do that from this game's Configure dialog after adding it.
            </p>
          </Show>
